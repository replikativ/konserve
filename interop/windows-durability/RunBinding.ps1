...
$source = $PSScriptRoot
javac -d $classes "$source/../../src/java22/konserve/internal/WindowsDirectorySync.java" "$source/DirectorySyncBindingProbe.java"
if ($LASTEXITCODE -ne 0) { throw 'FFM binding compilation failed' }
java --enable-native-access=ALL-UNNAMED -cp $classes konserve.internal.DirectorySyncBindingProbe $ScratchParent |
...
native-image -O0 --no-fallback --enable-native-access=ALL-UNNAMED `
    ...
    '-H:-UnlockExperimentalVMOptions' '--initialize-at-run-time=konserve.internal.WindowsDirectorySync$Native' `
    -cp $classes konserve.internal.DirectorySyncBindingProbe $executable |
