...
(defprotocol PEDNAsyncKeyValueStore
  ...
  (-exists? [this key sync?] "Checks whether value is in the store.")
  (-get-meta [this key sync?] "Fetch only metadata for the key.")
  (-get [this key sync?] "Returns the value stored described by key or nil if the path is not resolvable.")
  (-update-in [this key-vec meta-up-fn up-fn sync?]
  ...
  (-assoc-in [this key-vec meta-up-fn val sync?])
  (-dissoc [this key sync?]))

(defprotocol PBinaryAsyncKeyValueStore
  "Allows binary data byte array storage."
  (-bget [this key locked-cb sync?] "Calls locked-cb with a platform specific binary representation inside the lock, e.g. wrapped InputStream on the JVM and Blob in JavaScript. You need to properly close/dispose the object when you are done!")
  (-bassoc [this key meta-up-fn val sync?] "Copies given value (InputStream, Reader, File, byte[] or String on JVM, Blob in JavaScript) under key in the store."))
