...
(defn unsign [v]
  #?(:clj (mapv (fn [b](bit-and (int b) 0xff)) v)
     :cljs v))

(defn sign [v]
  #?(:clj v
     :cljs (mapv (fn [b](if (> b 128) (- b 256) b)) v)))
...
(defrecord AESEncryptor [serializer key]
  ...
  (-deserialize [_ read-handlers bytes]
    (let [salt #?(:clj (let [salt-array (byte-array salt-size)]
                         ...
                         (map int salt-array))
                   :cljs (seq (sign (seq (.slice bytes 0 salt-size)))))
  ...
  (-serialize [_ bytes write-handlers val]
    (let [unsigned-byte-offset 128
          salt (map #(int (- (inc %) unsigned-byte-offset)) (edn-hash (uuid)))]
      #?(:cljs
          (let [data (-serialize serializer bytes write-handlers val)
                iv (get-initial-vector salt key)
                bytes (encrypt (get-key salt key) (.from js/Array data) :iv (get-initial-vector salt key))
         ...
         (let [buffer-size (* 16 1024)
               ...
               ba (.toByteArray bos)
               iv (get-initial-vector salt key)
               encrypted ^bytes (encrypt (get-key salt key) ba :iv (get-initial-vector salt key))]
