...
#+BEGIN_SRC clojure
  ...
  ;; Create new store (errors if already exists for backends with strict semantics)
  (def store (k/create-store config))

  ;; Or connect to existing store (errors if doesn't exist for backends with strict semantics)
  ;; (def store (k/connect-store config))

  ;; Note: For backends without creation steps (Redis, LMDB, RocksDB, file),
  ;; create-store and connect-store behave identically.
...
#+BEGIN_SRC clojure
  ...
  (go
    (def my-idb-store (<! (k/connect-store {:backend :indexeddb
                                            :name "example-db"
                                            :opts {:sync? false}})))
