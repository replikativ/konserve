...
2. *Unified dispatch* - New polymorphic interface via =konserve.store= (see [[#h:unified-store-interface][Unified Store Interface]])
...
through the =konserve.store= namespace. All backends dispatch on a =:backend= key in
...
#+BEGIN_SRC clojure
  ...
  ;; Memory store
  (def store (store/connect-store {:backend :memory :opts {:sync? true}}))

  ;; File store (JVM and Node.js)
  (def store (store/connect-store {:backend :file
                                   :path "/tmp/konserve"
                                   :opts {:sync? true}}))

  ;; IndexedDB (browser)
  (go (def store (<! (store/connect-store {:backend :indexeddb
                                           :name "my-db"
                                           :opts {:sync? false}}))))

  ;; External backends (after requiring the backend module)
  (require '[konserve-s3.core])
  (def store (store/connect-store {:backend :s3
                                   :bucket "my-bucket"
                                   :region "us-east-1"
                                   :opts {:sync? true}}))
#+END_SRC
...
#+BEGIN_SRC clojure
  ;; Connect to a store
  (def store (store/connect-store config))

  ;; Create new empty store (equivalent to connect for most backends)
  (def store (store/empty-store config))

  ;; Clean up resources
  (store/release-store config store)

  ;; Delete underlying storage
  (store/delete-store config)
#+END_SRC
...
#+BEGIN_SRC clojure
  ...
  (def my-db (store/connect-store {:backend :file
                                   :path "path/to/folder"}))
#+END_SRC
...
=konserve.store= multimethod dispatch system.
...
#+BEGIN_SRC clojure
  ...
  (def s3-store (store/connect-store {:backend :s3
                                      :bucket "my-bucket"
                                      :region "us-east-1"
                                      :store-id "my-app"}))
#+END_SRC
